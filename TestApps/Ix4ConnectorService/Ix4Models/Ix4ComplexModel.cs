using System;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Text;
using System.Threading.Tasks;

namespace Ix4Models
{
    public class Ix4ComplexModel
    {
        public string ClientId { get; set; }
        public string ClientNo { get; set; }
        public string ArticleNo { get; set; }
        public string ArticleNo2 { get; set; }
        public string ArticleDescription { get; set; }
        public string ArticleDescription2 { get; set; }
        public string IdentityNo { get; set; }
        public string QuantityUnit { get; set; }
        public string EAN { get; set; }
        public string ProductCode { get; set; }
        public string ArticleGroup { get; set; }
        public string ArticleGroupDescription2 { get; set; }
        public string ArticleGroupFactor { get; set; }
        public string Weight { get; set; }
        public string DeliveryNo { get; set; }
        public string DeliveryDate { get; set; }
        public string DeliveryArea { get; set; }
        public string DeliveryPlace { get; set; }
        public string ReferenceNo { get; set; }
        public string OrderDate { get; set; }
        public string Positions { get; set; }
        public string PositionNo { get; set; }
        public string LoadType { get; set; }
        public string TargetLoadsCount { get; set; }
        public string ArticlesPerLoad { get; set; }
        public string TargetQuantity { get; set; }
        public string Charge { get; set; }
        public string SerialNo { get; set; }
        public string ExpiryDate { get; set; }
        public string ProjectNo { get; set; }
        public string SSCC { get; set; }
        public string Comment { get; set; }
        public string OrderNo { get; set; }
        public string ExtOrderNo { get; set; }
        public string State { get; set; }
        public string CustomerOrderNo { get; set; }
        public string CustomerNo { get; set; }
        public string CustomerText { get; set; }
        public string Prio { get; set; }
        public string DeliveryText { get; set; }
        public string ShipmentText { get; set; }
        public string PackageText { get; set; }
        public string CommissionText { get; set; }
        public string ShippingText { get; set; }
        public string ShipmentMode { get; set; }
        public string ShipmentDate { get; set; }
        public string CreatedDate { get; set; }
        public string CashOnDeliveryAmount { get; set; }
        public string OrderAmount { get; set; }
        public string SupplyAreaNo { get; set; }
        public string SupplyPlaceNo { get; set; }
        public string StockOutAreaNo { get; set; }
        public string StockOutPlaceNo { get; set; }
        public string TransportType { get; set; }
        public string DistributionCenter { get; set; }
        public string PaymentCondition { get; set; }
        public string Recipient { get; set; }
        public string FirstName { get; set; }
        public string Name { get; set; }
        public string AdditionalName { get; set; }
        public string Street { get; set; }
        public string City { get; set; }
        public string ZIPCode { get; set; }
        public string Country { get; set; }
        public string CompanyName { get; set; }
        public string ContractTittle { get; set; }
        public string ContractTelephone { get; set; }
        public string ContractFax { get; set; }
        public string ContractEmail { get; set; }
        public string AdditionalText { get; set; }
        public string NetPrice { get; set; }
        public string GrossPrice { get; set; }
        public string Tax { get; set; }

        public static PropertyInfo[] GetPropertiesList()
        {
            return typeof(Ix4ComplexModel).GetProperties();
        }
    }
}
